...
#Set up chat
```{r}
my_chat <- ellmer::chat_google_gemini()
chattr_use(my_chat)
library(ggplot2)
...
```{r}
# temp conversion function with conditional safety check using an if statement that halts execution via stop() if the entered value is below absolute zero (-273.15 degrees Celsius)
