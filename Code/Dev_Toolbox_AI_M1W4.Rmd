...
### if_then statments: if a condition is meet, do this command.
...
### if_else staments: used to sort data, such as if a conditon is not met start a second set of commands.
...
### else_if statements: allows additional conditional commands between the if and else

#### Example of if_else code:
```{r}
name <- "Nick"
if (name == "Nick"){
  last.name <- "Murray"
...
# Useful to set up warnings or alternate pathways such as by running base R script in code
```
...
# Can use these extensively when wrangling data
```
